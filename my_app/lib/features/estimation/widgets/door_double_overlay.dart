...
class DoorDoubleOverlay extends StatelessWidget {
  final int collarId;
  final String? selectedSection;
  final bool d46Enabled;
  final bool d52Enabled;

  const DoorDoubleOverlay({
    super.key,
...
class _DoorDoublePainter extends CustomPainter {
  final int collarId;
  final String? selectedSection;
  final bool d46Enabled;
  final bool d52Enabled;

  const _DoorDoublePainter({
    required this.collarId,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Rect innerRect = Rect.fromLTWH(
      outerRect.left + size.width * 0.03,
      outerRect.top + size.height * 0.045,
      outerRect.width - size.width * 0.06,
      outerRect.height - size.height * 0.09,
    );
    final double centerX = innerRect.center.dx;
    final double centerY = innerRect.center.dy;
    final double leftPanelCenterX = (innerRect.left + centerX) / 2;
    final double rightPanelCenterX = (centerX + innerRect.right) / 2;
    ...
    drawLabel(
      'W',
      Offset(leftPanelCenterX, innerRect.top + size.height * 0.05),
      highlight: highlightD50,
    );
    drawLabel(
      'W',
      Offset(rightPanelCenterX, innerRect.top + size.height * 0.05),
      highlight: highlightD50,
    );
    if (d52Enabled) {
      drawLabel(
        'T',
        Offset(leftPanelCenterX, centerY - size.height * 0.06),
        highlight: highlightD52,
      );
      drawLabel(
        'T',
        Offset(rightPanelCenterX, centerY - size.height * 0.06),
        highlight: highlightD52,
      );
    }
    drawLabel(
      'W',
      Offset(leftPanelCenterX, innerRect.bottom - size.height * 0.05),
      highlight: highlightD46 || (highlightD50 && !d46Enabled),
    );
    drawLabel(
      'W',
      Offset(rightPanelCenterX, innerRect.bottom - size.height * 0.05),
      highlight: highlightD46 || (highlightD50 && !d46Enabled),
    );
    ...
    drawLabel(
      'H',
      Offset(innerRect.left + size.width * 0.03, hY),
      highlight: highlightD50,
    );
    drawLabel(
      'H',
      Offset(centerX - size.width * 0.03, hY),
      highlight: highlightD50,
    );
    drawLabel(
      'H',
    ...
    drawLabel(
      'H',
      Offset(innerRect.right - size.width * 0.03, hY),
      highlight: highlightD50,
    );
  }

  @override
  bool shouldRepaint(covariant _DoorDoublePainter oldDelegate) {
